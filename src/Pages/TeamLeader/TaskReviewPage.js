...
const TaskReviewPage = () => {
  ...
  // Fetch submitted tasks for review
  const fetchSubmittedTasks = async (page = 1) => {
    try {
      setLoading(true);
      
      // Get tasks submitted by team members for this team leader to review
      const response = await axios.get(
        buildApiUrl(`/tasks/pending-reviews?page=${page}&limit=${tasksPerPage}`),
  ...
  useEffect(() => {
    fetchSubmittedTasks(currentPage);
  }, [currentPage]);

  // Handle navigation to task review details page
  const goToTaskReview = (taskId) => {
              ...
              <div className="row mb-4">
                <div className="col-12">
                  <div className="d-flex justify-content-between align-items-center">
                    <div>
                      <h4 className="mb-0">Task Review</h4>
                      <p className="text-muted mb-0">Review and approve submitted tasks from your team</p>
                    </div>
                ...
                <div className="col-12">
                  <div className="card border-0 shadow-sm">
                    <div className="card-header bg-white border-0">
                      <div className="d-flex justify-content-between align-items-center">
                        <h5 className="mb-0">Submitted Tasks for Review</h5>
                    ...
                    <div className="card-body">
                      {tasks.length > 0 ? (
                            ...
                            <tbody>
                              {tasks.map((task, index) => (
                                <tr key={task.id}>
                                  ...
                                  <td>
                                    <span>{task.submitted_by_name || task.assigned_to_name || 'Unknown'}</span>
                                  </td>
                                  <td>
                                  ...
                                  <td>
                                    <span className={`badge ${
                                      task.assigned_to_tester ? 'bg-success' : 'bg-warning'
                                    }`}>
                                      {task.assigned_to_tester_name ? task.assigned_to_tester_name : (task.assigned_to_tester ? `Tester ID: ${task.assigned_to_tester}` : 'Not Forwarded')}
                                    </span>
                                  </td>
                                  <td>
                                    <span className={`badge ${
                                  ...
                                  <td>
                                    <div className="d-flex gap-2">
                                      <button 
                                        ...
                                        onClick={() => goToTaskReview(task.id)}
                                      >
                        ...
                        <div className="text-center py-5">
                          ...
                          <p className="text-muted">Your team members haven't submitted any tasks for your review yet.</p>
                        </div>
                      )}
                      {/* Pagination */}
