...
const TeamLeaderCalendarTracker = () => {
  ...
  // Fetch calendar data for the team leader
  const fetchCalendarData = async (teamLeaderId, month, year) => {
    setLoading(true);
    try {
      // Fetch all tasks for the team leader (including daily routine tasks)
      const tasksResponse = await axios.get(
        buildApiUrl(`/tasks/user/${teamLeaderId}`),
        { headers: getAuthHeaders() }
      );
      
      // Fetch all logs for the team leader
      const logsResponse = await axios.get(
        buildApiUrl(`/tasks/logs/user/${teamLeaderId}`),
        { headers: getAuthHeaders() }
      );
      
      if (tasksResponse.data.success && logsResponse.data.success) {
        // Filter tasks to only show tasks assigned to the team leader, not assigned by the team leader
        const allTasks = tasksResponse.data.data.tasks || tasksResponse.data.data;
        const teamLeaderTasks = allTasks.filter(task => {
          // Task is assigned to the team leader either directly or as team leader
          return (task.assigned_to === teamLeaderId) || (task.assigned_to_team_leader === teamLeaderId);
        });
  ...
  const viewTaskDetails = (taskId) => {
    navigate(`/team-leader/my-task/${taskId}`);
  };
...
export default TeamLeaderCalendarTracker;