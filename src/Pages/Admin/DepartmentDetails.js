...
const DepartmentDetails = () => {
    ...
    const [departmentTasks, setDepartmentTasks] = useState([]); // New state for tasks
    const [loading, setLoading] = useState(true);
    const [showAddLeaderModal, setShowAddLeaderModal] = useState(false);
    const [showAssignManagerModal, setShowAssignManagerModal] = useState(false);
    const [showAssignTaskModal, setShowAssignTaskModal] = useState(false);
    const [showAddEmployeeModal, setShowAddEmployeeModal] = useState(false);
    const [showAssignTaskToEmployeeModal, setShowAssignTaskToEmployeeModal] = useState(false); // New modal state
    const [showMakeTeamLeaderModal, setShowMakeTeamLeaderModal] = useState(false); // New modal state
    ...
    const [activeSection, setActiveSection] = useState('teamLeaders'); // 'teamLeaders', 'employees', 'tasks'
    
    // Search states
    const [teamLeaderSearch, setTeamLeaderSearch] = useState('');
    const [employeeSearch, setEmployeeSearch] = useState('');
    const [taskSearch, setTaskSearch] = useState(''); // New search state for tasks
    
    // Pagination state for tasks
    const [currentPage, setCurrentPage] = useState(1);
    const [tasksPerPage] = useState(10);
    const [employeesPerPage] = useState(10); // Add this for employee pagination
    
    // New team leader form state
    ...
    // Check if user has admin role - useEffect runs after all state is initialized
    // useEffect(() => {
    //     if (!hasRole(['ADMIN'])) {
    ...
    // New state for assigning task to employee
    ...
    // New state for making employee a team leader
    ...
    const fetchDepartmentDetails = async () => {
        try {
            // Fetch department info from new endpoint
            ...
            if (deptResponse.data.success) {
                // Set department info and employees
                setDepartment({
            ...
            // Fetch team leaders for this department
            ...
            // Fetch department tasks directly using the department-specific endpoint
            ...
            if (tasksResponse.data.success) {
                // Set department tasks - only include main tasks (not subtasks)
            ...
            try {
                ...
                // Don't show error to user, just continue with empty designations
    ...
    // Fetch available team leaders for manager assignment (from this department only)
    ...
    // Handle new employee form input changes
    ...
    // Handle assign manager form input changes
    ...
    // Handle assign task form input changes
    ...
    // Handle assign task to employee form input changes
    ...
    // Handle make team leader form input changes
    ...
    // Handle file attachment for task assignment
    ...
    // Handle file attachment for task assignment to employee
    ...
    // Add new team leader to department
    const handleAddTeamLeader = async (e) => {
        ...
        // Validate phone number if provided (international format or 10-digit Indian)
        if (newTeamLeader.phone && newTeamLeader.phone.trim()) {
            const phoneRegex = /^(\+91[6-9]\d{9}|[6-9]\d{9})$/;
            const cleanedPhone = newTeamLeader.phone.trim().replace(/[\s\-\(\)]/g, '');
            if (!phoneRegex.test(cleanedPhone)) {
                toast.error('Invalid phone number format. Please use 10 digits (e.g., 9876543210) or international format (e.g., +919876543210)');
                return;
            }
        }
        ...
        try {
            // First create the user (in a real app, this would be a more complex process)
            // For now, we'll directly add to department_team_leaders table
    ...
    const handleAddEmployee = async (e) => {
        ...
        // Validate mobile number (international format or 10-digit Indian)
        if (!newEmployee.mobile || !newEmployee.mobile.trim()) {
            toast.error('Mobile number is required');
            return;
        }
        
        const mobileRegex = /^(\+91[6-9]\d{9}|[6-9]\d{9})$/;
        const cleanedMobile = newEmployee.mobile.trim().replace(/[\s\-\(\)]/g, '');
        if (!mobileRegex.test(cleanedMobile)) {
            toast.error('Invalid mobile number format. Please use 10 digits (e.g., 9876543210) or international format (e.g., +919876543210)');
            return;
        }
    ...
    const handleAssignTask = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                // Check if notification was sent (from response data if available)
                const notificationStatus = response.data.notificationStatus;
                
                if (notificationStatus && notificationStatus.success) {
                    toast.success('Task assigned successfully! WhatsApp notification sent to employee.');
                } else if (notificationStatus && notificationStatus.skipped) {
                    toast.warning(`Task assigned successfully, but notification was skipped: ${notificationStatus.reason || 'Unknown reason'}`);
                } else if (notificationStatus && !notificationStatus.success) {
                    toast.warning(`Task assigned successfully, but notification failed: ${notificationStatus.error || 'Unknown error'}`);
                } else {
                    // No notification status in response (notification is async)
                ...
                fetchDepartmentDetails(); // Refresh data to show any updates
            }
        } catch (error) {
    ...
    const handleAssignTaskToEmployee = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(
                buildApiUrl(API_ENDPOINTS.DEPARTMENTS.ASSIGN_TASK_TO_EMPLOYEE(departmentId)),
                formData,
                { 
                    headers: {
                        ...getAuthHeaders(),
    ...
    // Remove team leader from department
    ...
    // Fetch available team leaders when assign manager modal opens
    useEffect(() => {
                                ...
                                <div className="modal-body pt-2">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <small className="form-text text-muted">
                                            Optional: Enter 10-digit number or international format (+91XXXXXXXXXX)
                                        </small>
                                    </div>
                                    <div className="mb-3">
                                        ...
                                        {departmentDesignations && departmentDesignations.length > 0 ? (
                                            <>
                                                ...
                                                {(!newTeamLeader.designation || isNaN(newTeamLeader.designation)) && (
                                                    <input
                                                        type="text"
                                                        className="form-control rounded-3"
                                                        name="designation"
                                                        value={isNaN(newTeamLeader.designation) ? newTeamLeader.designation : ''}
                                                        onChange={handleNewTeamLeaderChange}
                                                        placeholder="Or enter a new designation"
                                                    />
                                                )}
                                            </>
                                        ) : (
                            ...
                            <form onSubmit={handleAssignManager}>
                                <div className="modal-body pt-2">
                                    ...
                                    <div className="alert alert-info">
                                        ...
                                        The selected team leader will be responsible for managing this department, 
                                        assigning employees to teams, and overseeing department operations.
                                    </div>
                                </div>
                                <div className="modal-footer border-0 pt-0">
                                    <button 
                            ...
                            <form onSubmit={handleAddEmployee}>
                                <div className="modal-body pt-2">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        />
                                        <small className="form-text text-muted">
                                            Enter 10-digit number or international format (+91XXXXXXXXXX)
                                        </small>
                                    </div>
            ...
            {showMakeTeamLeaderModal && (
                <div className="modal fade show d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content border-0 shadow">
                            <div className="modal-header border-0 pb-0">
                                <h5 className="modal-title fw-semibold">Promote Employee to Team Leader</h5>
                                <button 
                                    type="button" 
                                    className="btn-close" 
                                    onClick={() => setShowMakeTeamLeaderModal(false)}
                                ></button>
                            </div>
                            <form onSubmit={handleMakeTeamLeader}>
                                <div className="modal-body pt-2">
                                    <div className="mb-3">
                                        ...
                                        <select
                                            className="form-select rounded-3"
                                            name="employeeId"
                                            value={makeTeamLeaderForm.employeeId}
                                            onChange={handleMakeTeamLeaderChange}
                                            required
                                        >
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <input
                                            ...
                                            className="form-control rounded-3"
                                            name="designation"
                                            value={makeTeamLeaderForm.designation}
                                            onChange={handleMakeTeamLeaderChange}
                                            placeholder="Enter designation"
                                        />
                                    </div>
                                    <div className="alert alert-info">
                                        <i className="ri-information-line me-2"></i>
                                        This employee will be promoted to team leader and will be able to receive task assignments.
                                    </div>
                                </div>
                                <div className="modal-footer border-0 pt-0">
                                    <button 
            ...
            {showTaskDetailsModal && selectedTask && (
                <div className="modal fade show d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <div className="modal-dialog modal-dialog-centered modal-lg">
                        <div className="modal-content border-0 shadow">
                            <div className="modal-header border-0 pb-0">
                                <h5 className="modal-title fw-semibold">Task Details</h5>
                                <button 
                                    type="button" 
                                    className="btn-close" 
                                    onClick={() => {
                            ...
                            <div className="modal-body pt-2">
                                <div className="row">
                                    <div className="col-md-12 mb-3">
                                        <div className="d-flex align-items-center mb-3">
                                            <div className="avatar-sm me-3">
                                                <div className="avatar-title rounded-circle bg-primary bg-opacity-10 text-primary fs-4">
                                                    <i className="ri-task-line"></i>
                                                </div>
                                            </div>
                                            <div>
                                    ...
                                    <div className="col-md-6 mb-3">
                                        ...
                                        <span className={`badge ${
                                            selectedTask.status === 'COMPLETED' ? 'bg-success bg-opacity-10 text-success' : 
                                            selectedTask.status === 'IN_PROGRESS' ? 'bg-primary bg-opacity-10 text-primary' : 
                                            selectedTask.status === 'PENDING' ? 'bg-warning bg-opacity-10 text-warning' : 
                                            selectedTask.status === 'CANCELLED' ? 'bg-danger bg-opacity-10 text-danger' : 
                                            'bg-secondary bg-opacity-10 text-secondary'
                                        }`}>
                                    ...
                                    <div className="col-md-6 mb-3">
                                        <h6 className="fw-medium">Priority</h6>
                                        <span className={`badge ${
                                            selectedTask.priority === 'urgent' ? 'bg-danger bg-opacity-10 text-danger' : 
                                            selectedTask.priority === 'high' ? 'bg-warning bg-opacity-10 text-warning' : 
                                            selectedTask.priority === 'medium' ? 'bg-info bg-opacity-10 text-info' : 
                                            'bg-secondary bg-opacity-10 text-secondary'
                                        }`}>
                                    ...
                                    <div className="col-md-6 mb-3">
                                        <h6 className="fw-medium">Assigned To</h6>
                                        <div className="d-flex align-items-center">
                                            <div className="avatar-xs me-2">
                                                <div className="avatar-title rounded-circle bg-info bg-opacity-10 text-info fs-6">
                                                    <i className="ri-user-line"></i>
                                            ...
                                            <div>
                                                <p className="mb-0">{selectedTask.assigned_to_name || 'Unassigned'}</p>
                                                <p className="text-muted mb-0 small">
                                                    {selectedTask.assigned_to_type === 'TEAM_LEADER' ? 'Team Leader' : 
                                                     selectedTask.assigned_to_type === 'EMPLOYEE' ? 'Employee' : 
                                                     'Unknown'}
                                                </p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="col-md-6 mb-3">
                                        <h6 className="fw-medium">Due Date</h6>
                                        <p className="mb-0">
                                    ...
                                    {selectedTask.attachment_url && (
                                        <div className="col-md-12 mb-3">
                                            <h6 className="fw-medium">Attachment</h6>
                                            <div className="d-flex align-items-center">
                                                <div className="avatar-xs me-2">
                                                    <div className="avatar-title rounded-circle bg-primary bg-opacity-10 text-primary fs-6">
