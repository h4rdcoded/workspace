...
function Profile() {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const endpoint = isOwnProfile 
          ...
          : `${leadCrmApiURL}/users/${targetUserId}`;
    ...
    const fetchUserActivities = async () => {
      ...
      try {
        ...
        if (userData.role === 'ADMIN') {
          ...
          // For team leader, fetch only their own tasks (not tasks they assigned to others) and assigned tasks
          const [tasksResponse, assignedResponse] = await Promise.all([
            fetch(`${leadCrmApiURL}/tasks/my-tasks-with-subtasks?self_assigned=true`, {
              method: 'GET',
              headers: headers,
              credentials: 'include'
            }),
          ...
          if (tasksResponse.ok) {
            ...
            if (tasksData.success && tasksData.data) {
              // Use tasks from the data, not just completed/cancelled ones
          ...
          // For employee, fetch ALL their tasks (not just completed/cancelled)
          const response = await fetch(`${leadCrmApiURL}/tasks/user/${userData.id}`, {
            method: 'GET',
            headers: headers,
            credentials: 'include'
          });
  ...
  useEffect(() => {
    if (tlSelfTaskSearch) {
      const filtered = tlTasks.filter(task => 
        ...
        (task.parent_task_title && task.parent_task_title.toLowerCase().includes(tlSelfTaskSearch.toLowerCase())) ||
  ...
  const calculateProfileCompletion = (user) => {
    if (!user) return 0;
    let completed = 0;
    const total = 8; // Increased total fields
    
    if (user.name) completed++;
    if (user.email) completed++;
    if (user.mobile) completed++;
    if (user.role) completed++;
    if (user.is_active !== undefined) completed++;
    if (user.created_at) completed++;
    if (user.joining_date) completed++;
    if (user.employee_code) completed++;
            ...
            <div className="row g-4">
              <div className="col-auto">
                <div className="avatar-lg">
                  <img 
                    src={userData.profile_image ? `${staticFilesBaseURL}/${userData.profile_image.split('?')[0]}?t=${new Date().getTime()}` : "/assets/ip.png"} 
                    alt="user-img" 
                  ...
                  <div className="tab-pane active" id="overview-tab" role="tabpanel">
                    <div className="row">
                      <div className="col-xxl-3">
                        ...
                        <div className="card">
                          <div className="card-body">
                            <h5 className="card-title mb-3"> {userData.name || 'User'}'s Information </h5>           
                              ...
                              <table className="table table-borderless mb-0">
                                <tbody>
                                  ...
                                  <tr>
                                    ...
                                    <td className="text-muted">{formatDate(userData.created_at)}</td>
                                  </tr>
                                ...
                                <div className="col-md-6">
                                  <div className="d-flex mt-4">
                                    ...
                                    <div className="flex-grow-1 overflow-hidden">
                                      ...
                                      <h6 className="text-truncate mb-0">{userData.department_name}</h6>
                                    </div>
                                  </div>
                                </div>
                              )}
                              <div className="col-md-6">
                                <div className="d-flex mt-4">
                                  ...
                                  <div className="flex-grow-1 overflow-hidden">
                                    ...
                                    <h6 className="text-truncate mb-0">{getRoleDisplayName(userData.designation_title || 'System Admin')}</h6>
                                  </div>
                                </div>
                              </div>
                                ...
                                <div className="col-md-6">
                                  <div className="d-flex mt-4">
                                    ...
                                    <div className="flex-grow-1 overflow-hidden">
                                      ...
                                      <h6 className="text-truncate mb-0">{formatDateTime(userData.last_login)}</h6>
                                    </div>
                                  </div>
                                </div>
                              )}
                          ...
                          <div className="card-body">
                            {recentActivitiesLoading ? (
                              ...
                              <div className="text-center py-4">
                                ...
                                <p className="text-muted">{recentActivitiesError}</p>
                              </div>
                                  ...
                                  <tbody>
                                    {recentActivities.map((activity, index) => (
                                      <tr key={index}>
                                        ...
                                        <td>
                                          <h6 className="mb-1">{activity.description?.substring(0,30) }</h6>
                                          {activity.user_name && (
                                            <p className="text-muted mb-0">
                                              <i className="ri-user-line align-bottom me-1"></i>
                                              Related To {activity.user_name}
                                            </p>
                                          )}
                                        </td>
                                        <td>
                      ...
                      <div className="card-body">
                        {activitiesLoading ? (
                          ...
                          <div>
                            ...
                            {userData.role === 'TL' && (
                              <div>
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                  <h5 className="mb-0">
                                    {tlTaskView === 'self' ? 'Tasks & Subtasks I Am Working On' : 'Tasks I Assigned to Team Members'}
                                  </h5>
                                  ...
                                  <div>
                                    {tlTasks.length === 0 ? (
                                      <div className="text-center py-4">
                                        ...
                                        <p className="text-muted">No tasks are currently assigned to this team leader.</p>
                                      </div>
                                    ) : (
                                      <div>
                                          ...
                                          <div className="col-lg-6">
                                            <div className="search-box position-relative">
                                              <input
                                                ...
                                                placeholder="Search tasks & subtasks by title, description, assigner, or status..."
                                              ...
                                              {getPaginatedData(filteredTlSelfTasks).map((task) => (
                                                <tr key={task.id}>
                                                  ...
                                                  <td>
                                                    ...
                                                    task.status === 'CREATED' ? 'bg-warning' : 'bg-secondary'
                                                    }`}>
                                                ...
                                                {currentUser?.role === 'ADMIN' ? (
                                                  ...
                                                  <button 
                                                    className="btn btn-sm btn-outline-primary"
                                                    onClick={() => window.location.href = `/team-leader/task/${task.id}/view`}
                                                  >
                                                    <i className="ri-eye-line me-1"></i>
                                                    View
                                                  </button>
                                              ...
                                              {getPaginatedData(filteredTlAssignedTasks).map((task) => (
                                                <tr key={task.id}>
                                                  ...
                                                  <td>
                                                    ...
                                                    {task.description && (
                                                      <p className="text-muted mb-0 small">{task.description.substring(0, 30)}...</p>
                                                    )}
                                                  </td>
                                                  <td>
                                                    {task.parent_task_id ? (
                            ...
                            {userData.role === 'EMPL' && (
                              <div>
                                <h5 className="mb-3">All Tasks Assigned to Me</h5>
                                {employeeTasks.length === 0 ? (
                                  <div className="text-center py-4">
                                    <i className="ri-time-line text-muted" style={{fontSize: '3rem'}}></i>
                                    <h4 className="mt-3">No Tasks Found</h4>
                                    <p className="text-muted">No tasks have been assigned to this employee yet.</p>
                                  </div>
                                ) : (
                                  <div>
                                        ...
                                        <tbody>
                                          {getPaginatedData(filteredEmployeeTasks).map((task) => (
                                            <tr key={task.id}>
                                              ...
                                              <td>
                                                <h6 className="mb-0">{task.title?.substring(0, 55)+'...' || 'N/A'}</h6>
                                              </td>
                                              <td>
                                                                                                    {task.parent_task_id ? (
                                                      <span>{task.parent_task_title || `Task #${task.parent_task_id}`}</span>
